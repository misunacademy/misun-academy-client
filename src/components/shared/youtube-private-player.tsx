...
export function YoutubePrivatePlayer({ url, className }: YoutubePrivatePlayerProps) {
      ...
      const player = new window.YT.Player(target, {
        ...
        events: {
          onReady(e: any) {
            ...
            if (iframe) {
              // Overscan: make iframe taller & wider so YouTube's title bar (top)
              // and any bottom chrome are clipped by the overflow:hidden on the parent.
              // The extra 160px is split: -80px top offset hides the title/logo row.
              ...
              // THE FIX: Sandbox the iframe to block the "More videos" popup scripts entirely
              // We allow scripts (for the player to work) and same-origin, but block modals/popups
  ...
  const handleKeyboardControl = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    switch (e.key) {
      ...
      case 'J':
        e.preventDefault()
        seekToTime((playerRef.current?.getCurrentTime?.() ?? currentTime) - 5)
        resetControlsTimer()
        break
      case 'ArrowRight':
      case 'l':
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ pointerEvents: isPlaying ? 'auto' : 'none', borderRadius: 'inherit' }}
      />
      ...
      <div
        ...
        onClick={togglePlay}
      >
        {/* When paused: frosted-glass overlay blurs the iframe (killing "More Videos" readability)
             while still letting the video frame's colours show through — no black bars. */}
        {(!isPlaying && isReady) && (
          <div
        ...
        {/* Bottom controls bar */}
        <div
          className={`relative z-30 bg-linear-to-t from-black/90 via-black/40 to-transparent px-4 pb-3 pt-10 transition-opacity duration-300 ${
