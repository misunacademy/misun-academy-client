...
const closeMenu = (setIsOpen: (v: boolean) => void) => () => setIsOpen(false);
...
export default function MobileNavbar() {
    ...
    const safeUser = isHydrated ? user : undefined;
    ...
    return (
        <div className="relative z-[1001]">
            ...
            >
                <Link onClick={closeMenu(setIsOpen)} href="/" className="text-lg h-14 flex items-center border-b border-primary/20 font-bangla">হোম</Link>
                ...
                {isLearner && (
                    <Link onClick={closeMenu(setIsOpen)} href="/my-classes" className="text-lg h-14 flex items-center border-b border-primary/20 font-bangla text-primary">আমার ক্লাসগুলো</Link>
                )}

                {!safeUser ? (
                    <Link onClick={closeMenu(setIsOpen)} href="/auth" className="text-lg h-14 flex items-center border-b border-primary/20 font-bangla text-emerald-400">লগইন</Link>
                ) : (
                    <>
                        <div className="py-4 border-b border-green-500/20 flex items-center gap-3">
                            <div className="h-10 w-10 rounded-full overflow-hidden border border-green-500/40 flex items-center justify-center">
                                {user?.image ? (
                                    <Image src={user.image} alt={user.name || 'User profile'} width={40} height={40} className="h-full w-full object-cover" />
                                ) : (
                        ...
                        <Link onClick={closeMenu(setIsOpen)} href="/profile" className="text-lg h-14 flex items-center border-b border-primary/20 font-bangla">প্রোফাইল</Link>
                        {isLearner && (
                            <Link onClick={closeMenu(setIsOpen)} href="/enrollment-posters" className="text-lg h-14 flex items-center border-b border-primary/20 font-bangla">আপনার ভর্তি পোস্টারসমূহ</Link>
                        )}
                        {isLearner && (
                            <Link onClick={closeMenu(setIsOpen)} href="/my-classes/certificates" className="text-lg h-14 flex items-center border-b border-primary/20 font-bangla">সার্টিফিকেটসমূহ</Link>
                        )}
                        {isStaff && (
                            <Link onClick={closeMenu(setIsOpen)} href={dashboardPath} className="text-lg h-14 flex items-center border-b border-primary/20 font-bangla">ড্যাশবোর্ড</Link>
                        )}
                        <button type="button" onClick={handleLogout} className="text-left flex items-center gap-2 text-lg h-14 border-b border-primary/20 font-bangla text-red-400">
                ...
                <div className='flex space-x-4 pt-6 pb-2'>
                    <Link onClick={closeMenu(setIsOpen)} href="/checkout">
