...
function drawName(ctx: CanvasRenderingContext2D, config: PosterTemplate["config"], userName: string) {
  if (!userName) return;
...
function drawBatch(ctx: CanvasRenderingContext2D, config: PosterTemplate["config"], batchNo: string) {
  if (!batchNo) return;
