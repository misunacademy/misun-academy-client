...
function LessonFormDialog({ open, mode, moduleId, data, onClose, onSuccess }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess();
    } catch (err: unknown) { const error = err as { data?: { message?: string } }; toast.error(error?.data?.message || "Operation failed"); }
  };
...
function ModuleCard({
  ...
  const handleDeleteLesson = async () => {
    ...
    try {
      ...
      refetchLessons();
    } catch (err: unknown) { const error = err as { data?: { message?: string } }; toast.error(error?.data?.message || "Delete failed"); setDeleteLessonId(null); }
  };
...
export default function InstructorDashboardPage() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const instructorRecord = (dashData as any)?.course?.instructorId;
  ...
  const handleDeleteModule = async () => {
    ...
    try {
      ...
      refetch();
    } catch (err: unknown) { const error = err as { data?: { message?: string } }; toast.error(error?.data?.message || "Delete failed"); setDeleteModuleId(null); }
  };
  ...
  const handleMoveModule = async (fromIndex: number, toIndex: number) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || "Reorder failed");
      setOrderedModules(modules);
    }
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
          <StatCard icon={Users} label="Enrolled Students" value={(dashData as any)?.enrolledStudents ?? 0} color="bg-violet-500" />
          {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
          <StatCard icon={Layers} label="Active Batches" value={(dashData as any)?.activeBatches ?? 0} color="bg-blue-500" />
          {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
          <StatCard icon={BookOpen} label="Total Batches" value={(dashData as any)?.totalBatches ?? 0} color="bg-emerald-500" />
        </div>
      )}
